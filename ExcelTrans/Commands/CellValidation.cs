...
namespace ExcelTrans.Commands
{
    public struct CellValidation : IExcelCommand
    {
        ...
        public CellValidation(DataValidationKind validationKind, int row, int col, params string[] rules)
            : this(validationKind, ExcelService.GetAddress(row, col), rules) { }
        public CellValidation(DataValidationKind validationKind, int fromRow, int fromCol, int toRow, int toCol, params string[] rules)
            : this(validationKind, ExcelService.GetAddress(fromRow, fromCol, toRow, toCol), rules) { }
        public CellValidation(DataValidationKind validationKind, Address r, params string[] rules)
            : this(validationKind, ExcelService.GetAddress(r, 0, 0), rules) { }
        public CellValidation(DataValidationKind validationKind, Address r, int row, int col, params string[] rules)
            : this(validationKind, ExcelService.GetAddress(r, row, col), rules) { }
        public CellValidation(DataValidationKind validationKind, Address r, int fromRow, int fromCol, int toRow, int toCol, params string[] rules)
            : this(validationKind, ExcelService.GetAddress(r, fromRow, fromCol, toRow, toCol), rules) { }
        public CellValidation(DataValidationKind validationKind, string cells, params string[] rules)
        {
            When = When.Normal;
